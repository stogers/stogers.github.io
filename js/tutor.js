var Tutor = function() {
    ...
    var my = {
        ...
        html: {
            ...
            // Element to display the right sidebar img
            rightSidebar: null,
        },
    ...
    // Return:
    //   Unit object
    function unit(bookId)
    {
        return Units.main[bookId]
    }
    ...
    function displayUnitLinks()
    {
        ...
        for (var i = 0; i < Units.main[my.current.unitNo].chapters.length; i++) {
            var label = 'Chapter ' + (i + 1)
            if (Units.main[my.current.unitNo].chapterNames) {
                var namedLength = Units.main[my.current.unitNo].chapterNames.length
                if (i < namedLength) {
                    label = Units.main[my.current.unitNo].chapterNames[i]
                } else {
                    label = 'Chapter ' + (i - namedLength + 1)
                }
            }
    ...
    function boxedLink(label, selected, href, clickHandler)
    {
        ...
        wrapperElement.className = "tableofcontents"

        var divElement = document.createElement('div')
        divElement.style = "text-align: left; overflow: hidden; white-space: nowrap; border: none; padding: 0px; margin: 0px; background-color: inherited"
        ...
        if (selected) {
            ...
            wrapperElement.className = 'selected tableofcontents'
        } else {
            divElement.style = "text-align: left; overflow: hidden; white-space: nowrap; color: #666; border: none; padding: 0px; margin: 0px; background-color: inherited"
            divElement.className = "temp"
            var anchorElement = document.createElement('a')
            anchorElement.href = href
            Util.addChildren(divElement, label)
    ...
    function hideGuide()
    {
        ...
        Util.setChildren(guideLink, 'Show Keyboard')
        return false
    }
    ...
    {
        guide.style.display = ''
        Util.setChildren(guideLink, 'Hide Keyboard')
        return false
    }
    ...
    function resetSubunit()
    {
        ...
        console.log('state', my.current.state.toUpperCase(),
    ...
    function setSubunit(m, n)
    {
        ...
        var i = 0;
        for (var subunitTitle in my.current.unit.chapters) {
            my.current.subunitTitles.push("Chapter " + i)
            i += 1
        }
        my.current.subunitText = Units.main[m].chapters[n-1] 
    }
    ...
    function displayUnitTitle() {
        ...
        var unitNo = ''
        var space = ''
        var title = Units.main[my.current.unitNo].title
    ...
    {
        gKeyboard.highlightKey(my.current.targetChar)
    }
    ...
    function evaluateInput()
    {
        ...
        var lastChar = inputText[inputLength-1]
        console.log('pressing: ', lastChar)
        gKeyboard.pressKey(lastChar)
    ...
    function qtpi()
    {
        ...
        letterDiv.style.borderColor = '#F08'
